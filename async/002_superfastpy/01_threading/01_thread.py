...
if __name__ == '__main__':
    ...
    with t:
        # create all tasks
        threads = [Thread(target=task, args=(i,)) for i in range(1000000)]
        # start all threads
...
# 2) It is less effective if we have many more tasks than we can support concurrently because all
# of the tasks will run at the same time and could slow each other down.
# 3) It also does not allow results from tasks to be returned easily.