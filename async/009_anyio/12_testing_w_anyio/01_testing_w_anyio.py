...
async def test_something2(anyio_backend):
    ...
...
If you change the backends/options for the entire project, then put something like this in your top level 
conftest.py:"""
...
# If you want to specify different options for the selected backend, you can do so by passing a tuple of (backend
# name, options dict):
...
# If you need to run a single test on a specific backend, you can use @pytest.mark.parametrize (remember to add the
# anyio_backend parameter to the actual test function, or pytest will complain):
...
same fixture is being torn down or the test has finished running. As such, if no higher-order (scoped class or 
higher) async fixtures are used, a separate test runner is created for each matching test. Conversely, if even one 
async fixture, scoped higher than function, is shared across all tests, only one test runner will be created during 
the test session.
...
However, these context variables are not carried over to synchronous tests and fixtures, or to other async test runners.
...
function scoped async fixtures. Another significant difference with the AnyIO pytest plugin is that attempts to run 
the setup and teardown for async fixtures concurrently when their dependency graphs allow that."""
