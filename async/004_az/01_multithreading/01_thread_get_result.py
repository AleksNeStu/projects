...
# Extend the threading.Thread class and add a result member to your new class. Make sure to take into account
...
class ReturnValueThread(th.Thread):
    ...
    # Override the base class’s run() method: in addition to running the target function as expected (with its args
    # and kwargs intact), it has to store the target’s result in the new member result.
    def run(self):
    ...
    # Override the base class’s join() method: with args and kwargs intact, simply join() as in the base class but
...
# ReturnValueThread returns the result of the target function, our thread2 in the above example (the thread that
# times out) does not exit cleanly. In fact, it runs until the sleep() ends.
