...
async def main():
    async with trio.open_nursery as main_nursery:
        queue = trio.Queue()
        # Trio’s `Queue` and `Channel` objects are similar to those found in other concurrent programming languages,
        # such as the `Queue` class in Python’s `multiprocessing` module. They allow tasks to send and receive data
        # asynchronously, which can be useful for coordinating the behavior of multiple concurrent tasks.
