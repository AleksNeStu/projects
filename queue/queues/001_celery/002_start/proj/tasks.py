...
app = Celery(
    'tasks',
    # The first argument to Celery is the name of the current module. This is only needed so that names can be
    # automatically generated when the tasks are defined in the __main__ module.
    ...
    #
    # See Choosing a Broker above for more choices – for RabbitMQ you can use amqp://localhost, or for Redis you can
    # use redis://localhost.
    # https://docs.celeryq.dev/en/stable/userguide/tasks.html#task-result-backends
    backend='redis://localhost'
)

# The configuration can be set on the app directly or by using a dedicated configuration module. As an example you
# can configure the default serializer used for serializing task payloads by changing the task_serializer setting:
# app.conf.update(
...
# for libraries, as it enables users to control how their tasks behave. A centralized configuration will also allow
# your SysAdmin to make simple changes in the event of system trouble.
