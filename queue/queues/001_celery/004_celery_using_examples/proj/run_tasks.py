...
# must either define the environment variable DJANGO_SETTINGS_MODULE or call settings.configure() before accessing
# settings.
...
def get_all_child_tasks(parent_task_result):
    ...
    any grandchild tasks and so on. You can then use the AsyncResult objects in the list to get the status, result,
    or other information about the tasks.
    """
...
def run_tasks1v2():
    # It's important to note that the get and join methods will block the current thread of execution until the tasks
    # are completed. If you want to execute tasks asynchronously and wait for their results without blocking the
    # current thread, you can use the AsyncResult.wait method, which waits for a task to complete but does not block
    # the current thread.
    print("Run tasks 1v2")
