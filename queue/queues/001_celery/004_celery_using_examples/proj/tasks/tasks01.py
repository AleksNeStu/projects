...
"""
Use case description: fetch the five hundred hottest repositories from GitHub per chosen period (day, week, month), 
group them by topics, and export the result to the CSV file.
If we provide an HTTP service that will execute this feature triggered by clicking a button labeled “Generate Report,
” the application would stop and wait for the task to complete before sending an HTTP response back. This is bad. We 
want our web application to be fast and we don’t want our users to wait while our back-end computes the results. 
Instead of waiting for the results to be produced, we would rather queue the task to worker processes via a 
registered queue in Celery and respond with a task_id to the front-end. Then the front-end would use the task_id to 
query the task result in an asynchronous fashion (e.g., AJAX) and will keep the user updated with the task progress. 
Finally, when the process finishes, the results can be served as a file to download via HTTP.
"""
...
def fetch_hot_repos_v1(since, per_page, page):
    ...
    # [2022-12-11 20:08:42,614: ERROR/ForkPoolWorker-3] Chord 'c4f2828b-32be-4ff1-9401-99ae2d10a907' raised:
...
def produce_hot_repo_report_task_v1(period, ref_date=None):
    ...
    The produce_hot_repo_report_task function :
        ...
        2. fetches and joins hot repos for each of the 5 time periods (5 days) in that period, e.g., last week's hot
        repos are all fetched and joined together
...
'''
Tis task uses celery.canvas.group to execute five concurrent calls of fetch_hot_repos/3. Those results are waited for 
and then reduced to a list of repository objects. Then our result set is grouped by topic and finally exported into a 
generated CSV file under the MEDIA_ROOT/ directory.
'''
...
def fetch_hot_repos_group(group_params):
    ...
    # res =  result.get()
    # The RuntimeError('Never call result.get() within a task!') error is raised when the result.get() method is
    # called inside a Celery task. This is generally considered an anti-pattern because the result.get() method
    # blocks the current task until the result is ready, which can cause performance issues and lead to deadlocks.
    # print(res)
    # return res
    return job()
